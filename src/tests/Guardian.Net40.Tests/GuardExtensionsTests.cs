...
namespace Guardian.Tests
{
    ...
    public class GuardExtensionsTests
    {
        ...
        public void NullEnumeration()
        {
            // act
#if NETCOREAPP1_1
            var exception = Record.Exception(() => Guard.Against.NullOrEmpty((Expression<Func<IEnumerable<object>>>)null));
#else
        ...
        public void EmptyGuid()
        {
            ...
            // act
            var exception = Record.Exception(() => Guard.Against.Empty(() => guid));

            // assert
            exception.ShouldBeValid<ArgumentException>(ExceptionType.Empty).WithParameter("guid");
        }
